...
import { storeOAuthSession, consumeOAuthSession, storeXRefreshToken, getXRefreshToken } from './redis'

/**
 * Generate an OAuth 2.0 authorization URL with PKCE for X/Twitter.
 */
export async function generateAuthUrl(identityKey: string): Promise<{ url: string; state: string }> {
  const client = new TwitterApi({
    clientId: config.X_CLIENT_ID,
    clientSecret: config.X_CLIENT_SECRET,
  })

  const { url, codeVerifier, state } = client.generateOAuth2AuthLink(config.X_REDIRECT_URI, {
...
export async function handleCallback(
  ...
  logger.info({ identityKey: session.identityKey, userName }, 'X verification successful')
...
export async function shareCertification(
  ...
  if (!refreshToken) {
    throw Object.assign(
      new Error('No X authorization found. Please verify your X account first.'),
      { statusCode: 400 }
    )
  }
  ...
  const certTypeName = certType === 'email' ? 'email' : certType === 'phone' ? 'phone number' : 'X account'
  const tweetText = `I just verified my ${certTypeName} on SocialCert — my identity is now cryptographically linked to the blockchain.\n\n${frontendUrl}\n\n#SocialCert #BSV #DigitalIdentity`
